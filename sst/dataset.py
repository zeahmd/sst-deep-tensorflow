...
class SSTContainer:
    def __init__(self, root=False, binary=False):
        ...
        self.tokenizer = SSTTokenizer(
            corpus=self.train_set[0] + self.dev_set[0] + self.test_set[0]
        )
        del sst
            ...
            if self.binary:
                for tree in split_dataset:
                    if tree.to_labeled_lines()[0][0] != 2:
                        text.append(preprocess_sst(tree.to_labeled_lines()[0][1]))
                        sentiment.append(
            ...
            if self.binary:
                for tree in split_dataset:
                    for subtree in tree.to_labeled_lines():
                        if subtree[0] != 2:
                            text.append(preprocess_sst(subtree[1]))
                            sentiment.append(get_binary_label(subtree[0]))

            else:
                for tree in split_dataset:
                    for subtree in tree.to_labeled_lines():
                        text.append(preprocess_sst(subtree[1]))
                        sentiment.append(subtree[0])
